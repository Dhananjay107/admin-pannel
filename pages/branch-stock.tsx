      ...
      <div className="p-4 sm:p-6 space-y-6">
        ...
        >
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Branch-wise Stock (Super Admin)</h1>
              <p className="text-sm text-gray-600">Real-time stock levels, expiry risk, and low-stock alerts per pharmacy branch</p>
            </div>
          </div>
            ...
            <>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                <div className="bg-gray-50 rounded-lg p-4">
                  <p className="text-xs font-medium text-gray-500 uppercase">Total Branches</p>
                  <p className="text-2xl font-bold text-gray-900">{summary.totalBranches ?? branches.length}</p>
                </div>
                ...
                {branches.map((branch, idx) => (
                  ...
                  >
                    <div className="bg-gray-50 px-4 py-3 flex flex-wrap items-center justify-between gap-2">
                      <div>
                        <h2 className="font-semibold text-gray-900">{branch.pharmacyName}</h2>
                        {branch.address && <p className="text-xs text-gray-600">{branch.address}</p>}
                        {branch.phone && <p className="text-xs text-gray-500">Phone: {branch.phone}</p>}
                      </div>
                      <div className="flex flex-wrap gap-3">
                        ...
                        <span className="px-2 py-1 bg-gray-200 text-gray-800 rounded text-xs font-medium">
                          Value: ₹{(branch.totalStockValue || 0).toLocaleString()}
                        </span>
                    ...
                    <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                      {branch.lowStockItems?.length > 0 && (
                        <div>
                          <h3 className="text-sm font-semibold text-gray-700 mb-2">Low stock</h3>
                          <ul className="text-xs space-y-1">
                            ...
                            {branch.lowStockItems.length > 5 && (
                              <li className="text-gray-500">+{branch.lowStockItems.length - 5} more</li>
                            )}
                          </ul>
                        </div>
                      )}
                      {branch.expiringSoonItems?.length > 0 && (
                        <div>
                          <h3 className="text-sm font-semibold text-gray-700 mb-2">Expiring soon</h3>
                          <ul className="text-xs space-y-1">
                            {branch.expiringSoonItems.slice(0, 5).map((item, i) => (
                              <li key={i} className="flex justify-between">
                                <span className="text-gray-700">{item.medicineName} ({item.brandName || ""})</span>
                                <span className="text-orange-600">{new Date(item.expiryDate).toLocaleDateString()}</span>
                              </li>
                            ))}
                            {branch.expiringSoonItems.length > 5 && (
                              <li className="text-gray-500">+{branch.expiringSoonItems.length - 5} more</li>
                            )}
                          </ul>
                        </div>
                      )}
                      {(!branch.lowStockItems?.length && !branch.expiringSoonItems?.length) && (
                        <p className="text-sm text-gray-500 col-span-2">No alerts for this branch.</p>
                      )}
                    </div>
                  </motion.div>
                ))}
